                    ...
                    <div class="card" style="height: 450px; /* Set a fixed height for the card */">
                        <div class="card-header p-0 position-relative border-0">
                            <a href="{{ route('product-details', ['id' => $relatedProduct->id]) }}">
                                <img alt="card-image" class="d-block card-img-top" src="{{ $relatedProduct->image_path }}">
                            </a>
                        </div>
                        <div class="card-body service-details">
                            <span class="label-style label-style-3">
                                ...
                                @elseif($currentLanguage == 'ru')
                                    {{ $relatedProduct->category->name_ru }}
                                @elseif($currentLanguage == 'ge')
                                    {{ $relatedProduct->category->name_ge }}
                                @else
                                    {{ $relatedProduct->category->name_en }}
                                @endif
                            </span>
                            <a class="service-heading" href="{{ route('product-details', ['id' => $relatedProduct->id]) }}">{{ $relatedProduct->name }}</a>
                            <p>
                                <!-- Your additional content here -->
                            </p>
                        </div>
